...
import { FaBullhorn, FaSpinner, FaExternalLinkAlt, FaFilePdf, FaCalendarAlt } from 'react-icons/fa'; // Example icons
...
const parseDate = (dateStr) => {
  ...
  try {
    ...
    let date;
    if (Y_M_D_Dash.test(dateStr)) { // YYYY-MM-DD
        date = new Date(dateStr);
    } else if (D_M_Y_Slash.test(dateStr)) { // DD/MM/YYYY or MM/DD/YYYY - assumes MM/DD/YYYY for JS Date
        const parts = dateStr.split('/');
        date = new Date(`${parts[2]}-${parts[0]}-${parts[1]}`); // Convert to YYYY-MM-DD for consistency
    } else if (D_M_Y_Dash.test(dateStr)) { // DD-MM-YYYY
        const parts = dateStr.split('-');
...
const AnnouncementItem = ({ announcement, index }) => {
  ...
  const title = announcement["Announcement Title"] || `Announcement #${index + 1}`;
  const dateStr = announcement["Announcement Date"] || null;
  const summary = announcement["Announcement Summary"] || "View the announcement for more details.";
  ...
  return (
    <div className={styles.announcementItem}>
      ...
      {/* <p className={styles.itemType}>{type}</p> */} {/* Uncomment if you use Announcement Type */}
      ...
      >
        {isPdf ? <><FaFilePdf /> View PDF</> : <><FaExternalLinkAlt /> View Details</>}
      </a>
    </div>
  );
};
  ...
  return (
    <div className={styles.announcementsPage}>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageSubtitle}>Stay updated with the latest news and notices from the department.</p>
      </div>

      {announcements.length > 0 ? (
        <div className={styles.announcementsList}>
          {announcements.map((announcementData, index) => (
            <AnnouncementItem key={index} announcement={announcementData} index={index}/>
          ))}
        </div>
      ) : (
        <p className={styles.noAnnouncements}>No announcements posted yet. Please check back later.</p>
      )}
    </div>
  );
}
